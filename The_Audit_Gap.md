...
References [1] through [4] support the six-constraint Audit Gap framework (Section 3). References [63], [64], [330] through [347] are shared with the Biomimetic Gap Analysis paper and use the same numbering for cross-paper consistency, except for [347] (Matzinger 2002) which is unique to this paper. Only references cited in this paper are reproduced here; see the Biomimetic Gap Analysis paper for the full reference list supporting the biological mappings. References [348] through [356] support the substrate-drift monitoring evidence (Sections 2.4 and 10.7) and are unique to this paper; entries given without quoted titles are pending verification against the source PDFs.
...
[348] Chen L, Zaharia M, Zou J. "How Is ChatGPT's Behavior Changing Over Time?" arXiv:2307.09009. 2023. *Field anchor for LLM API behavioral monitoring: two snapshots months apart, accuracy-based comparison; origin of the recommendation to monitor continuously. Cited from the arXiv primary.*

[349] Chauvin et al. arXiv:2512.03816. ICLR 2026. *Hourly logprob tracking across 189 deployed LLM endpoints over four-plus months; 37 silent changes detected, all persistent and one-way, reversion unanalyzed; states the method cannot attribute a change to its cause.*

[350] Chauvin et al. B3IT. arXiv:2602.11083. ICML 2026. *Border inputs (tied top logits) proven maximally change-sensitive; total-variation-distance monitoring; requires at least four days of persistence before declaring a change, filtering transients by construction; 1 of 8 detected changes matched a vendor changelog.*

[351] Tschisgale, Wulff. arXiv:2602.15889. 2026. *Daily and weekly periodicity in GPT-4o outputs at 3-hour sampling cadence, roughly 20 percent of variance, serving-load hypothesis. Continuous reversible modulation: the closest published relative of the regime oscillation in Section 10.7.*

[352] Casper S, et al. "Black-Box Access is Insufficient for Rigorous AI Audits." arXiv:2401.14446. 2024. *Structural argument for the attribution ceiling: behavioral access alone cannot support the audits governance assumes.*

[353] Cai et al. arXiv:2504.04715. 2025. *Software-only statistical verification of deployed-model identity is brittle under production nondeterminism; points toward trusted execution environments. Cited for why zero attribution is structural rather than a tooling shortfall.*

[354] MD3 (Margin Density Drift Detection). arXiv:1704.00023. 2017. *Pre-LLM concept-drift detection built on classifier margin density: the boundary-as-sensor concept in the classical drift literature. The LLM monitoring lineage and this lineage do not cite each other.*
...
[356] Seth, Sankarapu. arXiv:2605.15164. May 2026. *Formalizes the audit gap as the interval between the access level independent verifiers hold and the access level governance evidence assumes, on a five-level access scale, assessed pre-deployment; descends from financial auditing's expectation-gap tradition. Independent coinage of the term relative to this project (Section 2.4).*

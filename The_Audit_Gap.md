...
The problem does not stop at the agent. The same argument applies one layer down, to the model serving infrastructure the agent runs on. An enterprise consuming a frontier model through an API holds behavioral evidence only. The model identifier the API echoes back is the serving layer's self-report, not a measurement of it, and no signal available from outside distinguishes a silent model change from a serving-stack change from a stationary model alternating between internal states. Section 10.7 reports twelve days of daily judgment-layer monitoring across five frontier model APIs: motion was detected under a constant model id, repeatedly and at high statistical confidence, and the cause was unattributable from outside. The motion detector problem extends below the agent to the substrate it runs on. *(Garcia check pending.)*
...
A separate research line monitors deployed LLM APIs for behavioral change under a constant model identifier. Chen, Zaharia, and Zou [348] anchored the field with two snapshots taken months apart and accuracy-based comparison, a design the Section 10.7 monitor inverts on every axis while executing that study's closing recommendation: monitor continuously. Chauvin et al. [349] tracked logprobs hourly across 189 endpoints for over four months and found 37 silent changes, all persistent and one-way, with reversion unanalyzed; the authors state the method cannot attribute a change to its cause. Their follow-on B3IT [350] proves that border inputs (inputs with tied top logits) are maximally change-sensitive and monitors total variation distance over them, requiring at least four days of persistence before declaring a change and filtering transients by construction; 1 of 8 detected changes matched any vendor changelog. B3IT is independent empirical confirmation that boundary inputs are the sensitive surface for change detection. The differences matter: B3IT engineers token-level probes where the Section 10.7 monitor designs semantic judgment equipoise; B3IT carries no clear-cut null class where the monitor's decisive items provide one; and the persistence filter discards by construction exactly the reversible oscillation phenomenon Section 10.7 reports. Tschisgale and Wulff [351] found daily and weekly periodicity in GPT-4o at 3-hour cadence, roughly 20 percent of variance, with serving load as the working hypothesis: reversible but continuous aggregate modulation, the closest published relative of discrete recurring states. Casper et al. [352] argue structurally that black-box access is insufficient for rigorous audit, and Cai et al. [353] show that software-only statistical verification is brittle under production nondeterminism, pointing toward trusted execution environments; together they position the attribution ceiling measured in Section 10.7 as structural, not a tooling shortfall. The boundary-as-sensor concept predates LLMs: margin-density drift detection [354] built classical concept-drift detectors on the same insight, and the two lineages do not cite each other. *(Garcia check pending.)*

One terminology note. The term audit gap was arrived at independently in this project's public working papers (divergence-series, initial commit 2026-04-10) before Seth and Sankarapu formalized the same phrase for a different object [356] (arXiv:2605.15164, May 2026). Their audit gap is an access interval: the distance between the access level governance evidence assumes and the access level independent verifiers actually hold, assessed pre-deployment on a five-level access scale. The Audit Gap in this paper is architectural: six constraints under which no mechanism inside the agent ensures fidelity between reasoning and output. The concepts compose rather than compete. The six constraints force audit outside the model; their interval describes what outside auditors then lack. The five-level access scale their interval runs over is adopted from Casper et al. [352]. Both audit gaps descend from the expectation-gap tradition in financial auditing (Liggio 1974, Porter 1993). *(Garcia check pending.)*
...
**Consequence:** The text output is not a faithful representation of the reasoning process. It is a product generated from the reasoning process, subject to the same editorial pressures that any product undergoes: inconvenient findings are omitted, uncomfortable conclusions are softened, the final output is polished for consumption. The Observability Gap (arXiv:2603.26942) documents the same property in coding agents: a many-to-one mapping from internal states to visible outputs that prevents reliable inference of process from product.
...
This demonstrates that the Compliance Override does not require sophisticated prompt injection. It requires only a false assertion delivered with conversational confidence. The agent's compliance training handles the rest. Two concurrent works frame this vulnerability architecturally. Trinity (Bhattarai and Vu, arXiv:2602.09947) argues that trustworthy agentic AI requires deterministic command/data separation so that user-supplied content cannot rewrite authorization state, the same architecture-over-prompt thesis advanced here. Semantic Laundering (Romanchuk and Bondar, arXiv:2601.08333), via its Theorem of Inevitable Self-Licensing, formalizes why tool boundaries do not confer epistemic warrant, an architectural-Gettier reading of the Compliance Override.
...
correlation natively. Concurrent identity-governance work targets this correlation gap directly: Authorization Propagation in Multi-Agent AI Systems (arXiv:2605.05440) tracks authorization across multi-agent delegation chains; OIDC-A (Benameur et al., arXiv:2509.25974) extends OpenID Connect to agent identities; the PEA Model (arXiv:2604.23646) adds intent-lineage tracking with cryptographic anchors; and SentinelAgent (Patil, arXiv:2604.02767) formalizes a delegation-chain calculus.
...
deployment performs. The propagation dynamics of this pattern are studied directly by Autonomous LLM Agent Worms (arXiv:2605.02812), which documents persistent, self-propagating worm behavior in file-backed agent ecosystems where shared state is the transmission medium.
...
name, as designed. It's just invoking the wrong one. Server-side enforcement at the tool-access boundary is the structural countermeasure: OpenPort (arXiv:2602.20196) specifies a gateway as the enforcement point for agent tool access, and MCPSHIELD (arXiv:2604.05969) provides a formal MCP security framework cataloguing tool-substitution and related attack surfaces across the MCP ecosystem.
...
accumulated agent context. Beyond Attention Stability (arXiv:2604.24512, 2026) names a related failure, the Attention Latch, attributing drift to information over-squashing and proposing a Self-Synthesizing Reasoning Protocol with an Architect/Executive split.
...
### 10.7 Substrate Drift: Twelve Days of Judgment-Layer Monitoring

The preceding subsections validate detection at the agent layer. This subsection reports evidence from one layer down. The Audit Gap argument does not assume the agent is the only opaque component in the stack. An enterprise consuming a frontier model through an API cannot audit the serving substrate either: the model identifier in every API response is the serving layer's self-report, not evidence about it. To measure what external behavioral evidence can and cannot establish about that substrate, a standing drift monitor ran for twelve days (2026-08-02 to 2026-08-13) against five production model APIs. The dataset and tooling are public in the self-context-calibration repository (Section 15).

The design: five frontier models (claude-haiku-4.5, claude-sonnet-4.6, gpt-5.6-terra, gemini-3.6-flash, deepseek-v4-flash) sampled daily at 13:00 UTC, K=10 samples per item per model, against a 68-item forced-choice enterprise security judgment bank: 23 of the 68 designed for equipoise (near-tied options, eq_ prefix) and the remaining 45 decisive, with a stable modal answer. The detection statistic is per-item total variation distance (TVD) from a frozen n=20 baseline taken August 2, compared against simulation-calibrated p99 alarm bands (0.40 for 311 of the 340 item-model cells, 0.45 for 28, 0.50 for 1). Twelve days produced 41,090 rows with zero unparsed responses. The echoed model id was constant in every row. The monitor costs roughly 20 USD per month to run. Five findings follow.

**Finding 1: reversible regime oscillation.** Distributions did not drift and stay drifted. They jumped between discrete states and returned. The sonnet fraud-scoring item sat at an 8A/12B baseline, jumped to exactly 10/0/0/0 on August 3, 4, 6, and 10 (9/1 on August 7, with the same-day rerun landing 10/0), and returned fully between excursions. The sonnet vulnerability-gating item went from unanimous A (20/20 baseline) to unanimous D (0/0/0/10) and back: TVD 1.0, the metric's maximum, with identical entropy (0 bits) at both ends, so any confidence- or entropy-based monitor scores the two states as equally healthy. The gpt offboarding item produced recurring exact vectors: 8/0/2/0 three times, 9/0/1/0 twice, plus intermediates. The haiku spend-anomaly item hit 0/6/0/4 exactly on August 12 and 13, a candidate step change unresolved at window close (it returned to baseline on August 10; the standing decision rule: a sustained hold is a step change and opens a re-baseline discussion; intermittent returns to baseline mean an alternator). Dwell in the excursion state is typically 1-2 daily observations, irregular, not periodic. The attractor states persist: the sonnet pair states have traded places since July 30, two-plus weeks at window close. One sub-day bound exists: gpt's disclosure item spiked twice and reverted within the roughly 15-minute rerun window. A literature sweep on 2026-08-13/14 searched for this phenomenon from six angles and found no prior report of discrete, recurring, reversible regime states in deployed LLM APIs; the closest published relative is continuous periodic modulation [351], and the persistence filters of [350] discard this class of event by construction.

**Finding 2: breaches concentrate entirely on designed equipoise.** All 29 breach entries in twelve days occurred on eq_ items, across 13 distinct item-model slots, all of them eq_. The 45 decisive items produced zero breaches in 2,700 slot-day checks; 97.8 percent of decisive daily distributions were identical to baseline. Under an honest per-slot null, eq_ items carry 38.1 percent of expected breach mass, not their 33.8 percent uniform share; on that null, the probability that all 29 entries land on eq_ items is 7.3e-13, and the more defensible distinct-slot framing gives 3.6e-6. The items designed to sit on judgment boundaries are the items that moved. This reproduces at the semantic judgment layer the boundary-sensitivity result that B3IT [350] established for token-level probes.

**Finding 3: recurrence and direction carry the evidence.** The load-bearing statistics come from exact enumeration, not Monte Carlo, and were adversarially verified by independent recomputation. The raw breach count is weak evidence and is stated as such: 29 entries against a null expectation of 19.7, P(>=29) = 0.029, and the count is null-model dependent (the unsmoothed null expects 0.61 breaches per twelve days, a 32x spread between the two nulls; the smoothed null is the conservative choice). The structure of the breaches is not weak. Chance scatters; this repeated. The 29 entries landed on 13 distinct slots against 19.2 expected under scatter. Seven slots breached two or more times against 0.537 expected (P = 1.5e-6). Five slots breached three or more times against 0.0101 expected (P = 8.0e-13). Two slots breached five times (P = 1.4e-12). All five of the three-plus-breach threads moved in the same direction on every excursion: combined P = 1.3e-16. A count-calibrated sensitivity check that inflates the null by 1.47x to match the observed count still leaves recurrence at 2.3e-10. One caveat is mandatory: the exact tails assume slot-day independence, and slots within a model-day share a scaffold (Section 14).

**Finding 4: detection without attribution, measured.** This is the Audit Gap instantiated as a number. Detection confidence from cross-day recurrence alone is on the order of 1e-12. Attribution evidence is zero bits: host, temperature_sent, and reasoning_detected are constant within each model across all 41,090 rows, and the echoed model id never moved. Whether the cause is a silent model update, a serving-stack change, load-dependent routing, or a stationary model alternating between internal states is not distinguishable from outside, at any monitoring budget. One statistic is deliberately demoted: 18 same-day rerun confirmations (EVENT) against 0.098 expected is nominally 9.8e-35, but that figure is conditional on probe and rerun being independent draws, and the item-level gap between them is 0.43 to 5.28 minutes (median 2.4 minutes); under total serving-state correlation the statistic degrades to P = 0.68. EVENT therefore certifies minutes-scale persistence only. Cross-day recurrence is the persistence evidence that matters.

**Finding 5: vendor phenotypes, stated modestly.** Twelve days support observed styles, not fixed vendor properties. The styles: gemini committed and frozen (18 of 23 eq_ baselines unanimous, 12 of 12 CLEAN days); deepseek splitting as designed, with honest sampling-like wander (overdispersion nearest 1); the Claude models and gpt on commit-and-flip (per-call unanimity, day-scale discrete flips). The consequence generalizes: per-call consistency is not temporal stability, and the most confident-looking response style is the least snapshot-auditable. Format carries a signal of its own: response length coupled to regime on sonnet's vulnerability-gating item (148 of 150 rows in lockstep) and weakly on haiku (9 long rows, only in the drifted window, only on non-modal answers), while sonnet's fraud-scoring item flipped with zero length change. Format is an independent observable, not a proxy for regime.

The obvious confounds are dead or disfavored. Time-of-day is dead for day-over-day comparison: since August 4 every run starts at 13:00:01 UTC with 1 second spread, per-model spread at most 18.2 minutes, and the Claude models within 3.1 minutes. A static 5-7 hour offset between the daily runs and the baseline window remains, but is empirically disfavored: an August 2 probe at 22:15 UTC, a different hour entirely, returned 68/68 CLEAN on all five models. The same day provides the clean-snapshot exhibit: the full bank was clean on all five models hours after baselining, and the first flip arrived the next day, so the baseline was not stale at birth.

Mechanism candidates exist in vendor gray literature. Anthropic's postmortem of three production serving defects from August-September 2025 [355] documents defects that were sticky per-user and missed by vendor evaluations; two of the three were fixed by rollback and one by a code fix. Anthropic's platform documentation states that infrastructure updates can "produce minor differences in observable behavior" under a fixed model id [357]. A 2025 Thinking Machines technical deep-dive on batch invariance [358] documents the numerics mechanism by which serving-batch composition alone changes outputs. None of these candidates can be confirmed or excluded from outside; that is the point.

Stated in the access-interval notation of Seth and Sankarapu [356]: for deployed-model change attribution, the verifier holds *behavioural* access while attribution requires *grey-box* access or above, and the measured content of behavioural-only verification is detection at the 1e-12 level with attribution at zero. Their carve-out that behavioural assurance is often adequate for decomposable, moderate-consequence properties holds only point-in-time; the temporal dimension is what breaks it. This is the Audit Gap of Section 3, measured. External behavioral evidence detects that the substrate moved, and cannot say why. The architectural conclusion of Section 4 carries unchanged one layer down: evidence capture must sit outside the component it audits, because inside the component there is nothing an external auditor can hold constant. *(Garcia check pending.)*
...
**3. Treat wrong configuration as worse than no configuration.** A monitoring system that gives false confidence is more dangerous than no monitoring at all. If agent behavioral monitoring is deployed, validate that it actually detects something. Run a tabletop against the TRIVIAL scenarios in Section 6. If the monitoring does not catch Scenario #1 (telemetry suppression) or Scenario #6 (privileged zone silence), it is not monitoring: it is a dashboard that makes the operator feel safe. This caution has empirical backing: the Verifier Tax (arXiv:2603.19328) shows runtime mediation carries a measurable, horizon-dependent cost, so a misconfigured verifier can be worse than none. For irreversible actions, Right-to-Act (arXiv:2604.24153) proposes a non-compensatory pre-execution boundary consistent with the Friction Constraint, where a failed safety check cannot be outweighed by other signals.
...
**Substrate-drift monitor caveats.** The Section 10.7 claims are bounded by design:

- Twelve days, five models, one item bank, one scaffold. The vendor phenotypes are observed styles, not fixed vendor properties.
- The attribution ceiling cuts inward as well as outward: model drift, stationary bistability, and sticky serving state cannot be distinguished from outside, and for some quiet equipoise items the frozen baseline itself may be the anomalous state (sign inversion; DESIGN_LIMITATIONS.md Limitation 4).
- Every exact tail probability assumes slot-day independence; slots within a model-day share a scaffold. The count-calibrated figure (2.3e-10) is a partial answer, not a resolution.
- The breach-count anomaly (P = 0.029) is a property of the smoothed null; the unsmoothed null expects 0.61 breaches per twelve days, a 32x spread. Smoothing is the conservative choice, and the spread is stated wherever the count is quoted.
- The detection floor is coarse: flat K=10 bands require roughly a 4-to-5-of-10 answer shift to alarm. Drift smaller than that is invisible, so decisive-item stability means stability above this floor, nothing stronger.
- The strict inequality at the band is load-bearing: a greater-or-equal comparison would raise the null breach mass 4.8x, and two observed near-misses sat exactly at 0.45.
...
References [1] through [4] support the six-constraint Audit Gap framework (Section 3). References [63], [64], [330] through [347] are shared with the Biomimetic Gap Analysis paper and use the same numbering for cross-paper consistency, except for [347] (Matzinger 2002) which is unique to this paper. Only references cited in this paper are reproduced here; see the Biomimetic Gap Analysis paper for the full reference list supporting the biological mappings. References [348] through [358] support the substrate-drift monitoring evidence (Sections 2.4 and 10.7) and are unique to this paper.
...
[348] Chen L, Zaharia M, Zou J. "How is ChatGPT's behavior changing over time?" arXiv:2307.09009. July 2023. *Field anchor for LLM API behavioral monitoring: two snapshots months apart, accuracy-based comparison; origin of the recommendation to monitor continuously.*

[349] Chauvin T, Le Merrer E, Taiani F, Tredan G. "Log Probability Tracking of LLM APIs." arXiv:2512.03816. ICLR 2026. *Hourly logprob tracking across 189 deployed LLM endpoints over four-plus months; 37 silent changes detected, all persistent and one-way, reversion unanalyzed; states the method cannot attribute a change to its cause.*

[350] Chauvin T, Lalanne C, Le Merrer E, Loubes JM, Taiani F, Tredan G. "Token-Efficient Change Detection in LLM APIs." arXiv:2602.11083. ICML 2026. *Introduces B3IT (Black-Box Border Input Tracking): border inputs (tied top logits) proven maximally change-sensitive; total-variation-distance monitoring; requires at least four days of persistence before declaring a change, filtering transients by construction; 1 of 8 detected changes matched a vendor changelog.*

[351] Tschisgale P, Wulff P. "Daily and Weekly Periodicity in Large Language Model Performance and Its Implications for Research." arXiv:2602.15889. February 2026. *GPT-4o outputs at 3-hour sampling cadence show daily and weekly periodicity, roughly 20 percent of variance, serving-load hypothesis. Continuous reversible modulation: the closest published relative of the regime oscillation in Section 10.7.*

[352] Casper S, Ezell C, Siegmann C, et al. "Black-Box Access is Insufficient for Rigorous AI Audits." arXiv:2401.14446. 2024. *Structural argument for the attribution ceiling: behavioral access alone cannot support the audits governance assumes. Source of the five-level access taxonomy that Seth and Sankarapu [356] build on.*

[353] Cai W, Shi T, Zhao X, Song D. "Are You Getting What You Pay For? Auditing Model Substitution in LLM APIs." arXiv:2504.04715. April 2025. *Software-only statistical verification of deployed-model identity is brittle under production nondeterminism; points toward trusted execution environments. Cited for why zero attribution is structural rather than a tooling shortfall.*

[354] Sethi TS, Kantardzic M. "On the Reliable Detection of Concept Drift from Streaming Unlabeled Data." arXiv:1704.00023. 2017. *Proposes MD3 (Margin Density Drift Detection): pre-LLM concept-drift detection built on classifier margin density, the boundary-as-sensor concept in the classical drift literature. The LLM monitoring lineage and this lineage do not cite each other.*

[355] Anthropic. "A postmortem of three recent issues." September 17, 2025. https://www.anthropic.com/engineering/a-postmortem-of-three-recent-issues. *Vendor postmortem of three August-September 2025 production serving defects: sticky per-user, missed by vendor evaluations; two of three fixed by rollback, one by a code fix. Gray-literature anchor for the Section 10.7 mechanism space.*

[356] Seth P, Sankarapu VK. "Position: Behavioural Assurance Cannot Verify the Safety Claims Governance Now Demands." arXiv:2605.15164. May 2026. *Formalizes the audit gap as the interval between the access level independent verifiers hold and the access level governance evidence assumes, on a five-level access scale adopted from Casper et al. [352], assessed pre-deployment; descends from financial auditing's expectation-gap tradition. Independent coinage of the term relative to this project (Section 2.4).*

[357] Anthropic. "Model IDs and versioning." Claude Developer Platform documentation. https://platform.claude.com/docs/en/about-claude/models/model-ids-and-versions. *States: "Occasionally, infrastructure updates produce minor differences in observable behavior even when the model ID and weights have not changed." Vendor acknowledgment of the fixed-model-id caveat quoted in Section 10.7.*

[358] He H, Thinking Machines Lab. "Defeating Nondeterminism in LLM Inference." Thinking Machines Lab: Connectionism. September 10, 2025. https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/. DOI: 10.64434/tml.20250910. *Technical analysis attributing output variation under fixed weights to batch-composition-dependent numerics; a mechanism candidate for the serving-side regime effects in Section 10.7.*
